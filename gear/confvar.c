...
ConfVar* confvar_validate(const char *file, char** error) {
    ...
    *error = NULL;
    do {
    	...
    	if( !confvar_long(cv, CONF_HEARTBEATPERIOD, NULL) ) {
            asprintf(error, "configuration file with key %s has invalid number", CONF_HEARTBEATPERIOD);
            break;
    	}

        f = 0;
        if( !confvar_float(cv, CONF_HEARTBEATTIMEOUT, &f) ) {
            asprintf(error, "configuration file with key %s has invalid number", CONF_HEARTBEATTIMEOUT);
            break;
        }
        if(f<=0) {
            asprintf(error, "configuration file with key %s value must be > 0", CONF_HEARTBEATTIMEOUT);
            break;
    	}
    } while(false);
...
size_t confvar_absent(const ConfVar *head, char *absent_key, size_t buflen) {
    const char *mandatory[] = {CONF_PIDFILE, CONF_LOGPATH, CONF_SAWANG, CONF_HEARTBEATPERIOD, CONF_HEARTBEATTIMEOUT, CONF_GONGGO, NULL};
    const char **p;
